...
// GET - List all first level dispositions
export async function GET(req: NextRequest) {
  return withAuthAndRole(['ADMIN', 'SUPERVISOR', 'AGENT'], async () => {
    try {
      const dispositions = await prisma.firstLevelDisposition.findMany({
        orderBy: { name: 'asc' },
      });
...
// POST - Create new first level disposition (ADMIN only)
export async function POST(req: NextRequest) {
  return withAuthAndRole(['ADMIN'], async () => {
    try {
      ...
      const disposition = await prisma.firstLevelDisposition.create({
